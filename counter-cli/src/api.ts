...
export const increment = async (counterContract: DeployedCounterContract): Promise<FinalizedTxData> => {
  logger.info('Recording age verification on-chain...');
  const finalizedTxData = await counterContract.callTx.increment();
  return finalizedTxData.public;
};

/**
 * Verify age and record on-chain.
 * 1. Check age >= 18 client-side (the birth year never leaves the client)
 * 2. If valid, call the contract's increment circuit to record the verification
 */
export const verifyAge = async (counterContract: DeployedCounterContract, birthYear: number): Promise<FinalizedTxData> => {
  const currentYear = 2026;
  const age = currentYear - birthYear;
  if (age < 18) {
    throw new Error('Age verification failed: Must be 18 or older.');
  }
  logger.info(`Age verified locally (age=${age}). Recording on-chain...`);
  return increment(counterContract);
};
...
export const displayVerifiedCount = async (
  ...
  const verifiedCount = await getCounterLedgerState(providers, contractAddress);
  if (verifiedCount === null) {
    logger.info(`There is no age verification contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Total verified users: ${Number(verifiedCount)}`);
  }
