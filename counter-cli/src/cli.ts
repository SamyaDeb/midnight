...
║              ─────────────────────────                       ║
║              Privacy-preserving age verification Demo        ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
`;
...
/** Build the age verification actions menu. */
const verificationMenu = (dustBalance: string) => `
${DIVIDER}
  Age Verification${dustBalance ? `                     DUST: ${dustBalance}` : ''}
${DIVIDER}
  [1] Verify age (prove you are 18+)
  [2] Display total verified users
  [3] Exit
${'─'.repeat(62)}
> `;
 ...
 * can verify their age or query total verified users.
 */
const mainLoop = async (providers: CounterProviders, walletCtx: api.WalletContext, rli: Interface): Promise<void> => {
  ...
  while (true) {
    const dustLabel = await getDustLabel(walletCtx.wallet);
    const choice = await rli.question(verificationMenu(dustLabel));
    switch (choice.trim()) {
      case '1':
        try {
          const birthYearStr = await rli.question('Enter your birth year (e.g. 1990): ');
          const birthYear = parseInt(birthYearStr.trim(), 10);
          await api.withStatus('Verifying age (ZK proof generation)', () => api.verifyAge(contract, birthYear));
          console.log('  ✓ Age verified successfully! You are confirmed 18+\n');
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Age verification failed: ${msg}\n`);
        }
        break;
      case '2':
 ...
 *   4. Enter the contract deploy/join and age verification loop
...
export const run = async (config: Config, _logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      // Step 4: Enter the age verification interaction loop
